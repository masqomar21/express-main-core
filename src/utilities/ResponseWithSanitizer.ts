/**
 * Enhanced Response Utility with built-in sanitization
 * Wraps ResponseData to automatically sanitize sensitive data
 */
...
import { SanitizeOptions, sanitizeResponse, sanitizeUser } from './ResponseSanitizer'

export const ResponseDataWithSanitizer = {
  /**
  ...
  serverError: (res: Response, error: any, message = 'Internal server error'): Response => {
    ...
    if (error instanceof UploadError && error.type !== 'UPLOAD_ERROR') {
      return ResponseDataWithSanitizer.badRequest(res, error.message)
    }
  ...
  /**
   * Response with user data (automatically uses sanitizeUser)
   * @param res - Express response object
   * @param user - User object to be sent
   * @param message - Optional message for the response
   * @return {Response} - Express response object with JSON data
   */
  okUser: <T extends Record<string, any>>(
    res: Response,
    user: T,
    message = 'Success',
  ): Response => {
    const sanitizedUser = sanitizeUser(user)
    return res.status(StatusCodes.OK).json({ status: StatusCodes.OK, message, data: sanitizedUser })
  },

  /**
   * Response with array of users (automatically sanitizes each user)
   * @param res - Express response object
   * @param users - Array of user objects
   * @param message - Optional message for the response
   * @return {Response} - Express response object with JSON data
   */
  okUsers: <T extends Record<string, any>>(
    res: Response,
    users: T[],
    message = 'Success',
  ): Response => {
    ...
    return res
      ...
      .json({ status: StatusCodes.OK, message, data: sanitizedUsers })
  },
...
export default ResponseDataWithSanitizer
